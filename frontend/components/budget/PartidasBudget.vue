          ...
          <div class="container-row" slot-scope="{ node }">
            <div class="row">
              ...
              <div class="col-sm col-50">
                <template v-if="!hasChildren(node)">{{ getPartida(node)?getPartida(node).unit:"-" }}</template>
              </div>
              <div class="col-sm text-right col-100">
                <template v-if="!hasChildren(node)">{{ getPartida(node)?getPartida(node).quantity:0|decimal("0.00") }}</template>
              </div>
              <div class="col-sm text-right col-100">
                <template v-if="!hasChildren(node)">{{ getPartida(node)?getPartida(node).get_unit_subtotal:0|currency(budget.currency) }}</template>
              </div>
              <div class="col-sm text-right col-100">
              ...
              <div>
                <b-dropdown variant="link" size="lg" no-caret>
                  ...
                  <b-dropdown-item @click="copiar(node)" href="#">Copiar</b-dropdown-item>
  ...
  export default {
      ...
      methods: {
        ...
        getPartida(node){
          ...
          let partida = this.$store.getters['budget/getFilteredBudgetPartida'](node.data.id)
          // Guardamos el subtotal de la partida para usarlo en el calculo de los subtotales por ramas
          this.$set(node.data, "get_subtotal", partida[0]?partida[0].get_subtotal:0)
          return partida ? partida[0] : null
        }
      }
  }
</script>
