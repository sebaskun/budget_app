<template>
  <tr>
      ...
      <td class="text-right">{{item.get_cost_unit|currency(budget.currency)}}</td>
      <td class="text-right">{{item.get_overhead_cost|currency(budget.currency)}}</td>
      <td class="text-right">{{getRatio(item.get_overhead_cost)|decimal('0.00%')}}</td>
      <td class="text-right">
        <template v-if="!editing">
          <a @click="edit">
