      ...
      <b-row class="bd-content">
        <b-col md="12" class="my-1">
          <b-row>
            ...
            <!-- <b-col md="3">
              <b-form-group horizontal id="class_subcontract_group"
                label="Clase de subcontract:"
...
export default {
    ...
    data() {
        return {
          ...
          fields: [
              ...
              // {key:'get_costo_transporte', label: 'Costo Transporte', sortable: false},   ----- Adicionar màs adelante
    ...
    created(){
      ...
      this.$store.dispatch('subcontractBudget/requestBudgetSubcontracts', payload)
    ...
    computed: {
      ...
      items(){
        let items = this.$store.getters['subcontractBudget/getAvailableBudgetSubcontracts']
    ...
    methods: {
      ...
      onSubmit(evt){
        ...
        if(isValid){
          this.$store.dispatch(`subcontractBudget/${this.mode}BudgetSubcontract`, this.model).then(response => {
            this.$swal({
              position: 'top-end',
              toast: true,
              type: 'success',
      ...
      onRemoveItem(payLoad){
        ...
        .then((willDelete) => {
          if (willDelete.value) {
            this.$store.dispatch('subcontractBudget/deleteBudgetSubcontract', payLoad)
              .then(response=>{
                this.$swal({
                  position: 'top-end',
                  toast: true,
