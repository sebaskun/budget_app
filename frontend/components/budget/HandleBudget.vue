<template>
  <div>
    <b-row align-h="end">
      <!-- <b-dropdown id="ddown1" class="float-right" variant="link" no-caret right size="lg">
        <template slot="button-content">
          <icon name="ellipsis-v"></icon>
        </template>
        <b-dropdown-item @click="onDeleteButtonClick">Eliminar presupuesto...</b-dropdown-item>
        <b-dropdown-item @click="onCopyButtonClick">Copiar presupuesto...</b-dropdown-item>
      </b-dropdown> -->
    </b-row>
    <div v-if="isLoading">
        <div>Loading...</div>
    ...
    <div>
      ...
      <b-row>
        <b-col lg="12">
          <b-button size="sm" @click="onPrimaryButtonClick" variant="primary">{{primaryButtonLabel}}</b-button>
          <template v-if="!isEditing">
            <b-button size="sm" @click="onCopyButtonClick" variant="success">Copiar</b-button>
            <b-button size="sm" @click="onDeleteButtonClick" variant="danger">Eliminar...</b-button>
          </template>
          <b-button size="sm" @click="onCancelButtonClick" variant="link">Cancelar</b-button>
        </b-col>
      </b-row>
      <br><br><br>
    </div>
...
export default {
    ...
    methods: {
      ...
      onCopyButtonClick(){
        const url=`/budget/api/budgets/${this.budget.id}/copy`
        this.$http.get(url)
          .then((response) => {
            this.$store.dispatch('getBudgetList')
            this.$emit('cancelButtonClicked');
      ...
      onDeleteButtonClick(){
        let vm = this
        this.$dialog.confirm(`Desea eliminar el presupuesto ${this.budget.title}`, {
          loader: true
        })
          .then((dialog) => {
